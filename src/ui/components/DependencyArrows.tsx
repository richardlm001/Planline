...
export function DependencyArrows({ dayToPixel }: DependencyArrowsProps) {
  ...
  const { taskRowIndex: taskIndexMap, totalVisualRows } = useMemo(
  ...
  const arrows = dependencies.map((dep) => {
    ...
    // Hide arrows involving children of collapsed groups
    if (fromTask.groupId && collapsedGroupIds.has(fromTask.groupId)) return null;
    if (toTask.groupId && collapsedGroupIds.has(toTask.groupId)) return null;
    ...
    // Both tasks should be visible at this point; if not, skip
    return null;
  });
